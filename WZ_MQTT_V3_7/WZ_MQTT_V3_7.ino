...
版本：V3.7
2019年6月22日00:15更新看门狗逻辑，断网处理逻辑，修复断网卡死，修复无限循环配网的问题，基本实现稳定运行。 WZ指令更换WIFI待优化。
...
void loop() {
  ...
  if (stringComplete) {
    if (inputString.indexOf("wz")) {
      if (inputString.indexOf("Change_Wifi")> 0) { //收到复位指令   这里没搞定  20190622
//      EEPROM.begin(512);
//      CONFIG buf;
//      Serial.println("SmartConfig Success");
//      Serial.printf("SSID:%s\r\n", WiFi.SSID().c_str());
//      Serial.printf("PSW:%s\r\n", WiFi.psk().c_str());
//      strcpy(buf.ssid,  WiFi.psk().c_str());
//      strcpy(buf.password, WiFi.SSID().c_str());
//      EEPROM.put<CONFIG>(0, buf);
//      EEPROM.commit();
      Serial.println("WZ will reset after 10s!!!");
        ESP.wdtDisable();     //关闭看门狗
        while (1) {};        //死循环复位
      }
      /************************************json 数据处理********************************************/
